...
server.get(
  ...
  async (
    ...
    // return `Get user: ${user.name} with age ${user.age}`;
    // return `Get user: ${user.name} with age ${user.age} and id ${user.id}`;
...
server.put<UpdateUserRequest>('/update/:id', async (request, reply) => {
  ...
  // return `Updated user with ID: ${id}, name: ${name || "unchanged"}, age: ${age || "unchanged"}`;
  return reply
    .status(200)
...
server.post('/login', async (request, reply) => {
  ...
  try {
    ...
    reply.status(500).send({ error: error }); // Kalau ada error lain
  }
});
